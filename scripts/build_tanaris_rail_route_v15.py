...
DEFAULT_SOURCE = Path("data/routes/generated/tanaris_terrain_coverage_cycle_v14_rail.json")
DEFAULT_OUTPUT = Path("data/routes/generated/tanaris_terrain_coverage_cycle_v15_rail.json")
DEFAULT_REMOVE_START = 184
DEFAULT_REMOVE_END = 210
DEFAULT_START_XY = (46.60, 76.81)
DEFAULT_END_XY = (44.73, 76.18)


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build Tanaris rail V15 by removing the complete live-observed "
            "western Gaping Chasm spur."
        )
    )
...
def generate_v15_bridge(
    ...
    *,
    ...
    return tuple(coords), validation
    ...
    *,
    ...
    for item in result["route_loop"]:
        if item.get("source") == "live_observed_gaping_v14_west_bypass":
            item["source"] = "live_observed_gaping_v15_spur_bridge"
        if "v13_index" in item:
            item["v14_index"] = item.pop("v13_index")
    for node in result["route_nodes"]:
        node["source"] = "tanaris_v15_rail_node"
        plan = node.get("terrain_access_plan")
        if isinstance(plan, dict) and "v13_attachment_route_index" in plan:
            plan["v14_attachment_route_index"] = plan.pop(
                "v13_attachment_route_index"
            )
            plan["v14_resume_route_index"] = plan.pop("v13_resume_route_index")
    result["name"] = "Tanaris terrain-aware full rail cycle v15 Gaping spur removed"
    result["source"] = {
        "route": str(source.get("name") or "unknown"),
        "removed_v14_indexes": [remove_start, remove_end],
        "excluded_runtime_node_ids": sorted(excluded_node_ids),
    }
    result["route_override"] = {
        "reason": (
            "V14 still entered the visibly named The Gaping Chasm at "
            "48.11,73.69. V15 removes the entire eastbound U-shaped spur and "
            "bridges directly west before the live boundary."
        ),
        "observed_boundary_coordinate": {"x": 48.11, "y": 73.69},
        "removed_v14_indexes": [remove_start, remove_end],
        ...
        "excluded_runtime_node_ids": sorted(excluded_node_ids),
        "preserved_v14_override": source.get("route_override", {}),
    }
    result["metrics"]["excluded_v15_hazard_node_count"] = len(excluded_node_ids)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    detour, validation = generate_v15_bridge(planner)
    excluded = frozenset(
